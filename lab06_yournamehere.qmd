...
b. Compute the average diameter at breast height for all Mt Tabor trees.  Explain why this value is a parameter ($\mu$) and not a statistic ($\overline{x}$).
...
d.  Construct and graph a sampling distribution for the mean diameter for samples of size 10.  For the sampled means of tree diameter, also compute the mean and standard error.
